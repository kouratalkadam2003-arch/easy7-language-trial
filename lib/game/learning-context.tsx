...
import { getPhrase, PHRASES, type Phrase } from '@/lib/learning/phrases'
import {
  applySM2Result,
  buildDueQueue,
  createSM2State,
...
import { loadProgress, saveProgress } from '@/lib/learning/progress-store'

export type SessionCard = { state: SM2State; phrase: Phrase }
...
type LearningState = {
  ...
  due: number
  /** بطاقات جلسة المراجعة الحالية (يقتصر على المستحقة) */
  buildReviewSession: () => SessionCard[]
  /** تسجيل نتيجة بطاقة (SM-2 Rating) */
  answerCard: (phraseId: string, rating: Rating) => void
...
export function LearningProvider({ children }: { children: ReactNode }) {
  ...
  const [reviews, setReviews] = useState<SM2State[]>([])
  const [notebookOpen, setNotebookOpen] = useState(false)
  const hydrated = useRef(false)

  // تحميل التقدم المحفوظ (متصفح فقط — M4 لاحقاً)
  useEffect(() => {
  ...
  const answerCard = useCallback((phraseId: string, rating: Rating) => {
    setReviews((prev) =>
      prev.map((r) => (r.phraseId === phraseId ? applySM2Result(r, rating) : r)),
    )
  }, [])

  const buildReviewSession = useCallback((): SessionCard[] => {
    return buildDueQueue(reviews)
      .map((state) => {
...
export function speakEnglish(text: string) {
  ...
  try {
    ...
    utterance.onerror = (e) => {
      ...
      // صمت الأخطاء حتى لا تكسر الجلسة
    }
