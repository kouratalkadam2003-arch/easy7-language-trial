...
export default function VillageGrid() {
  ...
  useEffect(() => {
    if (gridRef.current) {
      setTimeout(() => {
        const centerMarker = gridRef.current?.querySelector('.center-marker');
        if (centerMarker) {
          centerMarker.scrollIntoView({ block: 'center', inline: 'center', behavior: 'smooth' });
        }
      }, 150);
    }
  }, []);
    ...
    <div className="village-container">
      <div className="village-grid-wrapper">
        ...
        >
          ...
          {/* Center Marker for scrolling */}
          <div 
            className="center-marker absolute pointer-events-none" 
            style={{ 
              left: 10 * CELL_SIZE, 
              top: 10 * CELL_SIZE, 
              width: CELL_SIZE, 
            ...
            return (
              <div
                ...
                style={{
                  ...
                  backgroundColor: b.isBuilding ? 'rgba(0,0,0,0.3)' : `${def.color}33`,
                  borderColor: selectedBuildingId === b.id ? '#fbbf24' : `${def.color}88`,
                }}
                onClick={(e) => {
                  e.stopPropagation();
