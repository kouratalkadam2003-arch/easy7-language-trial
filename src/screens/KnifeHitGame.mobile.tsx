        ...
        recognition.onresult = (event: any) => {
          ...
          const processAttempt = (textToTest: string) => {
            ...
            if (isMatch) {
              ...
              setSpeechFeedback({
                message: `نطق رائع ومتقن! 🎯 (${displayTarget})`,
                isCorrect: true,
                heard: bestResult.heardText || textToTest,
              ...
              if (textToTest.length >= Math.min(3, displayTarget.length)) {
                synthSounds.playClang();
                // Track mistake in cross-stage tracker
                useLessonTrackerStore.getState().recordKnifeHit(displayTarget, false);
                engineRef.current.shakeDuration = 10;
                engineRef.current.shakeIntensity = 4;
                setSpeechFeedback({
                  message: `❌ سمعنا: "${bestResult.heardText || textToTest}" — المطلوب: "${displayTarget}"`,
                  isCorrect: false,
                  heard: bestResult.heardText || textToTest,
                  expected: displayTarget
                });
