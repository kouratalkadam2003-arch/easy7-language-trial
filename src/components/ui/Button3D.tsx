...
type ButtonVariant = 'primary' | 'secondary' | 'danger' | 'ghost' | 'gray' | 'success';

interface Button3DProps extends Omit<React.ComponentPropsWithoutRef<typeof motion.button>, 'onDrag'> {
...
export const Button3D = React.forwardRef<HTMLButtonElement, Button3DProps>(
  ({ className, variant = 'primary', fullWidth = false, children, disabled, ...props }, ref) => {
    ...
    const variantStyles: Record<ButtonVariant, string> = {
      ...
      success: 'bg-green-500 text-white shadow-green-700 hover:bg-green-600',
    };
